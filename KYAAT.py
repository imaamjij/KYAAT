...
import mediapipe as mp #Detect the presence and location of multiple obj
import pyautogui       #Detect the action form mouse or keyboard
import math
...
class HandRecog:
    ...
    # Function to find Kyaat Encoding using current finger_state.
    ...
    def get_Kyaat(self):
        """
        returns int representing Kyaat corresponding to Enum 'Gest'.
        sets 'frame_count', 'ori_Kyaat', 'prev_Kyaat', 
        ...
        current_Kyaat = Gest.PALM
        if self.finger in [Gest.LAST3,Gest.LAST4] and self.get_dist([8,4]) < 0.05:
            if self.hand_label == HLabel.MINOR :
                current_Kyaat = Gest.PINCH_MINOR
            else:
                current_Kyaat = Gest.PINCH_MAJOR
            ...
            ratio = dist1/dist2
            if ratio > 1.7:
                current_Kyaat = Gest.V_GEST
            else:
                if self.get_dz([8,12]) < 0.1:
                    current_Kyaat =  Gest.TWO_FINGER_CLOSED
                else:
                    current_Kyaat =  Gest.MID
            
        else:
            current_Kyaat =  self.finger
        
        if current_Kyaat == self.prev_Kyaat:
            self.frame_count += 1
        else:
            self.frame_count = 0

        self.prev_Kyaat = current_Kyaat

        if self.frame_count > 4 :
            self.ori_Kyaat = current_Kyaat
        return self.ori_Kyaat

# Executes commands according to detected Kyaats
class Controller:
    """
    Executes commands according to detected Kyaats.
    ...
    flag : bool
        true if V Kyaat is detected
    grabflag : bool
        true if FIST Kyaat is detected
    pinchmajorflag : bool
        true if PINCH Kyaat is detected through MAJOR hand,
        on x-axis 'Controller.changesystembrightness', 
        on y-axis 'Controller.changesystemvolume'.
    pinchminorflag : bool
        true if PINCH Kyaat is detected through MINOR hand,
        on x-axis 'Controller.scrollHorizontal', 
        on y-axis 'Controller.scrollVertical'.
    pinchstartxcoord : int
        x coordinate of hand landmark when pinch Kyaat is started.
    pinchstartycoord : int
        y coordinate of hand landmark when pinch Kyaat is started.
    pinchdirectionflag : bool
        true if pinch Kyaat movment is along x-axis,
        otherwise false
    prevpinchlv : int
        stores quantized magnitued of prev pinch Kyaat displacment, from 
        starting position
    pinchlv : int
        stores quantized magnitued of pinch Kyaat displacment, from 
        starting position
    framecount : int
    ...
    def pinch_control_init(hand_result):
        """Initializes attributes for pinch Kyaat."""
    ...
    def handle_controls(Kyaat, hand_result):  
        """Impliments all Kyaat functionality."""      
        x,y = None,None
        if Kyaat != Gest.PALM :
            x,y = Controller.get_position(hand_result)
        
        # flag reset
        if Kyaat != Gest.FIST and Controller.grabflag:
            Controller.grabflag = False
            pyautogui.mouseUp(button = "left")

        if Kyaat != Gest.PINCH_MAJOR and Controller.pinchmajorflag:
            Controller.pinchmajorflag = False

        if Kyaat != Gest.PINCH_MINOR and Controller.pinchminorflag:
            Controller.pinchminorflag = False

        # implementation
        if Kyaat == Gest.V_GEST:
            Controller.flag = True
            pyautogui.moveTo(x, y, duration = 0.1)

        elif Kyaat == Gest.FIST:
            if not Controller.grabflag : 
                Controller.grabflag = True
                pyautogui.mouseDown(button = "left")
            pyautogui.moveTo(x, y, duration = 0.1)

        elif Kyaat == Gest.MID and Controller.flag:
            pyautogui.click()
            Controller.flag = False

        elif Kyaat == Gest.INDEX and Controller.flag:
            pyautogui.click(button='right')
            Controller.flag = False

        elif Kyaat == Gest.TWO_FINGER_CLOSED and Controller.flag:
            pyautogui.doubleClick()
            Controller.flag = False

        elif Kyaat == Gest.PINCH_MINOR:
            if Controller.pinchminorflag == False:
                Controller.pinch_control_init(hand_result)
                Controller.pinchminorflag = True
            Controller.pinch_control(hand_result,Controller.scrollHorizontal, Controller.scrollVertical)
        
        elif Kyaat == Gest.PINCH_MAJOR:
...
'''
----------------------------------------  Main Class  ----------------------------------------
    Entry point of Kyaat Controller
'''


class KyaatController:
    """
    ...
    ----------
    gc_mode : int
        indicates weather Kyaat controller is running or not,
    ...
    def __init__(self):
        ...
        KyaatController.gc_mode = 1
        KyaatController.cap = cv2.VideoCapture(0)
        KyaatController.CAM_HEIGHT = KyaatController.cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
        KyaatController.CAM_WIDTH = KyaatController.cap.get(cv2.CAP_PROP_FRAME_WIDTH)
    ...
    def classify_hands(results):
        ...
        if KyaatController.dom_hand == True:
            KyaatController.hr_major = right
            KyaatController.hr_minor = left
        else :
            KyaatController.hr_major = left
            KyaatController.hr_minor = right
